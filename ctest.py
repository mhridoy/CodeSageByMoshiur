...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS code_snippets 
                 (id TEXT PRIMARY KEY, code TEXT, output TEXT)''')
    conn.commit()
    conn.close()

def save_code(code, output):
    unique_id = str(uuid.uuid4())
    conn = sqlite3.connect('code_database.db')
    c = conn.cursor()
    c.execute("INSERT INTO code_snippets (id, code, output) VALUES (?, ?, ?)", 
              (unique_id, code, output))
    conn.commit()
    conn.close()
    return unique_id

def get_code(unique_id):
    conn = sqlite3.connect('code_database.db')
    c = conn.cursor()
    c.execute("SELECT code, output FROM code_snippets WHERE id = ?", (unique_id,))
    data = c.fetchone()
    conn.close()
    return data if data else (None, None)
def share_code():
    with st.spinner("Saving and generating link..."):
        unique_id = save_code(c_plus_code, result)
    st.success(f"Code shared! Access it at `your_app_url?code_id={unique_id}`")
...
def main():
    ...
    if "code_id" in query_params:
        code_id = query_params["code_id"][0]
        code, output = get_code(code_id)
        if code and output:
            st.code(code, language='cpp')
            st.code(output, language='bash')
        else:
            st.error("Code snippet not found.")
        return
    ...
    with tab1:
        ...
        if st.button("Run C++ Code"):
            ...
            result = execute_cpp_code(c_plus_code, user_input)
            st.code(result, language='bash')
            share_button = st.button("Share Code")
            if share_button:
                share_code()
...
def display_youtube_channel():
        ...
        st.video("https://www.youtube.com/watch?v=qsqYEGav6mU")
